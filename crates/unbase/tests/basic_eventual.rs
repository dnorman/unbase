...
fn basic_eventual() {
    ...
    async_std::task::block_on(async {
        ...
        let slab_c = Slab::new_ephemeral(&net);

        // Basic sanity tests

        assert_eq!(slab_a.peer_slab_count(), 2, "Slab A Should know two peers");
        assert_eq!(slab_b.peer_slab_count(), 2, "Slab B Should know two peers");
        assert_eq!(slab_c.peer_slab_count(), 2, "Slab C Should know two peers");
