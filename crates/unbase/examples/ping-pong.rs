...
/// rather than gossip (once the record has been made resident) or index convergence (prior to the record being
/// located).
#[async_std::main]
async fn main() {
...
async fn player_one() {
    ...
    let slab = Slab::new_ephemeral(&net1);
...
async fn player_two() {
    ...
    let slab = Slab::new_ephemeral(&net2);
