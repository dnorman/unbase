...
impl<'a> TransmitterArgs<'a> {
    pub fn get_slab_id(&self) -> SlabId {
        match self {
            &TransmitterArgs::Local(ref s) => s.my_ref.id().clone(),
    ...
    pub fn send(&self, from: &SlabRef, memoref: MemoRef) {
        ...
        match self.internal {
            Local(ref tx) => {
                ...
                tx.lock()
                  .unwrap()
                  .send((from.clone(), memoref))
                  .expect("local transmitter send")
            },
            Dynamic(ref tx) => tx.send(from, memoref),
            Blackhole => {
                warn!("WARNING! Transmitter Blackhole transmitter used. from {:?}, memoref {:?}",
                      from, memoref);
            },
        }
    }
