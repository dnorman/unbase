...
#[derive(Clone)]
pub(super) struct SlabStore(Arc<SlabStoreInner>);
...
impl SlabStore {
    ...
    pub(super) fn initialize_new_slab(basedir: &std::path::Path, slab_id: &SlabId, keypair: Keypair) -> Result<Self,StorageError> {
        let pathbuf = basedir.join(format!("./unbase-{}.sled", slab_id));
        let db = sled::open(pathbuf.as_path() )?;
...
//    }
//
//    pub fn put_slab(&self, slab_id: &SlabId, slabbuf: SlabBuf<EntityId, MemoId>) -> Result<(), StorageError> {
...
//    pub fn get_slab(&self, slab_id: &SlabId) -> Result<SlabBuf<EntityId, MemoId>, StorageError> {
