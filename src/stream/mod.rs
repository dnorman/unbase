...
//! Stream, used for intermediate/compact representations
//!
//! Uses offsets inside a given Stream to more compactly represent data rather than via duplication.
...
//pub (crate) use self::receiver::NetworkReceiver;

//pub (crate) trait NetworkStream {
//
//}
...
/// `MemoId`, offset of the Memo's subject id, list of slab presence offsets wherein this memo
/// is purportedly present, list of slab presence offsets wherein this memo is peered but not present
#[derive(Serialize,Deserialize,Clone)]
...
impl NetworkBuffer{
    pub fn new( from_slabref: SlabRef ) -> Self {
        // TODO what are we doing with from_slabref?
        let len = 1;
        NetworkBuffer{
    ...
    pub fn add_memoref_peerset_and_memo(&mut self, memoref: &MemoRef, peerset: &MemoPeerList, memo: &Memo ) {
        ...
        let body: MemoBodyBuffer = match memo.body {
            MemoBody::SlabPresence{ ref s, ref p, ref r } => {
                ...
                MemoBodyBuffer::SlabPresence {
                    ...
                    r: r.iter().map(|memoref| {
                        let subj_offset = self.add_subject(memoref.subject_id.unwrap());
            ...
            MemoBody::Peering(ref memoid, ref subject_id, ref peerlist) => {
                unimplemented!()
//                let subj_offset = self.add_subject(subject_id);
...
//                )
        ...
        let subj_offset = self.add_subject(memo.subject_id.unwrap());
        ...
        self.segments.push( Segment::Memo(buf) );
    }
...
//            }).collect();
//
//            if let Some(&mut Segment::MemoRef(
...
//        }
    ...
    fn add_subject(&mut self, subject_id: SubjectId) -> SubjectOffset {
        match self.subject_offsets.binary_search_by(|x| x.1.cmp(&subject_id) ){
            ...
            Err(i) =>{
                self.segments.push(Segment::Subject(subject_id));
    ...
    fn add_memoref_and_peerset (&mut self, memoref: &MemoRef, subj_offset: SubjectOffset, peerset: &MemoPeerSet) -> SegmentId {
        match self.memoref_offsets.binary_search_by(|x| x.1.cmp(memoref) ){
            ...
            Err(i) =>{
                ...
                self.segments.push(Segment::MemoRef(MemoRefBuffer(memoref.memo_id.clone(), Some(memopeer_states), subj_offset)));
                let seg_id = (self.segments.len() - 1) as u16;  // TODO: Truncation
    ...
    fn add_memoref(&mut self, memoref: &MemoRef, subj_offset: SubjectOffset ) -> MemoRefOffset {
        match self.memoref_offsets.binary_search_by(|x| x.1.cmp(memoref) ){
            ...
            Err(i) =>{
                self.segments.push(Segment::MemoRef(MemoRefBuffer(memoref.memo_id.clone(), None, subj_offset)));
                let seg_id = (self.segments.len() - 1) as u16;  // TODO: Truncation
    ...
    fn add_slabref(&mut self, slabref: &SlabRef) -> SegmentId {
        match self.slabref_offsets.binary_search_by(|x| x.1.cmp(slabref) ){
            ...
            Err(i) =>{
                self.segments.push(Segment::SlabRef(slabref.slab_id));
    ...
    fn add_slabpresence(&mut self, presence: SlabPresence, slabref_offset: SlabRefOffset) -> SegmentId {
        self.segments.push(Segment::SlabPresence(
...
//            match segment {
//                Segment::Subject(subject_id) => {},
//                Segment::SlabRef(_) => {},
//                Segment::MemoRef(_) => {},
//                Segment::Memo(memobuf) => {
//                    // memobuf.extract(&segments, receiver)
//                },
//                Segment::SlabPresence(_) => {},
//            }
//        }
////       receiver.put_memo()
//        unimplemented!()
...
//
//    pub fn get_local_slab_handle_by_id <'a> (&'a mut self, slab_id: &SlabId) -> Option<&'a Slab> {
...
impl MemoBuffer {
    fn extract (self, segments: &Vec<Segment>, receiver: StoreHandle ) -> Result<(),Error> {
...
//        if let Some(&Segment::MemoRef(MemoRefBuffer(ref memo_id, _, ref subj_seg_id))) = segments.get(mr_seg_id as usize) {
//            my_memo_id = memo_id.clone();
//            memo_subj_seg_id = subj_seg_id;
//
//            if let Some(&Segment::Subject(ref subject_id)) = segments.get(*subj_seg_id as usize) {
//                my_subject_id = *subject_id;
//            }else {
//                return Err(Error::Buffer(BufferError::DecodeFailed));
//            }
//        }else {
//            return Err(Error::Buffer(BufferError::DecodeFailed));
//        }
...
//            if let Some(&Segment::MemoRef(MemoRefBuffer(ref parent_memo_id, _, ref parent_subj_seg_id))) = segments.get(parent_mr_seg_id as usize) {
